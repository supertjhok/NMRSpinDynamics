...
```text
Added opt-in received-signal noise support. `NoiseSpec` now supports seeded
white noise for CPMG spectra and imaging k-space, output-referred probe-noise
density for tuned, untuned, and matched CPMG spectra, probe-colored
tuned/matched imaging noise before echo integration, matched-filter SNR
estimation from repeated noisy spectra, and noisy image post-processing via
`use_noisy=True`. Added direct time-domain echo noise, richer metadata with
realized noise/SNR fields, and repeated-trial imaging noise statistics.
Existing deterministic fields remain noiseless, with noisy companion fields
populated only when requested. Added a user-facing received-signal noise
example.
Ran focused noise workflow tests, example smoke tests, the fast smoke suite,
and full unittest discovery under the external Conda/SciPy runtime.
Result: OK (focused noise/SNR/image helpers: 6 passed; examples/smoke slice:
36 passed; Conda full: 152 passed, 2 skipped)
```

2026-06-18:

```text
Added 1D and separable 2D inverse Laplace transform analysis helpers for T1,
T2, T1-T2, and D-T2 kernels with adjustable Tikhonov regularization. Added a
plotting example that generates synthetic data with configurable SNR levels
and shows recovered distributions.
Ran compile checks, focused ILT tests, example smoke tests, and full unittest
discovery under the external Conda/SciPy runtime.
Result: OK (ILT: 6 passed; examples: 4 passed; Conda full: 158 passed,
2 skipped)
```
...
```text
Added SNR-informed inverse-Laplace regularization selection. The selector scans
candidate Tikhonov strengths, estimates the expected residual norm from RMS
SNR, and applies a discrepancy-principle rule while preserving the full
candidate trace. The plotting example now supports `--auto-regularization`.
Ran focused analysis/example tests, compile checks, and full unittest discovery
under the external Conda/SciPy runtime.
Result: OK (analysis/examples slice: 13 passed; Conda full: 161 passed,
2 skipped)
```
...
```text
Added the initial `spin_dynamics.motion` module for Lagrangian isochromat
motion. The module includes 2D field-map interpolation, particle initialization
from density maps, deterministic advection, seeded Brownian diffusion,
reflect/periodic/clip boundaries, local-B1 RF rotations, free-precession
updates through sampled B0, and receive summation through local B1 receive
sensitivity. Added plotting examples for linear motion through static B0/B1 maps
and diffusion-driven CPMG echo attenuation in a static gradient.
...
```text
Added deterministic radiation-damping back-action coupled to tuned/matched
probe parameters. The FID workflow validates the instant model against the
analytic Section 10.2.5 envelope; tuned and matched finite CPMG trains can opt
into nonlinear damping through a radiation_damping mapping. Added circuit
ringdown, detuning/phase controls, optional operator-split damping during RF
pulse matrices, sample magnetization presets, sensitivity-squared damping
weights, console examples, plotting examples, and API documentation.
Ran focused radiation-damping tests, example smoke tests, compile checks, fast
smoke tests, and diff whitespace checks under the bundled Codex Python runtime.
Result: OK (focused radiation damping: 13 passed; examples: 4 passed; smoke
tier: 32 passed, 4 skipped)
```

2026-06-18:

```text
Added an idealized pumped NMR maser example on top of the radiation-damping
feedback model. `simulate_nmr_maser` lets longitudinal relaxation pump toward
an inverted `pump_mz`; the examples compare below-threshold decay with
above-threshold gain and inversion depletion. The plotting example exposes the
instant high-Q limit by default and can switch to the finite-ringdown circuit
model. The plot defaults were retuned to include a strong 16x-threshold pump so
the top trace reaches nonlinear saturation on the default time axis.
Ran focused radiation-damping tests, example smoke tests, compile checks, the
fast smoke tier, diff whitespace checks, and the non-plotting maser example
under the bundled Codex Python runtime. The plotting example CLI/help path
passes; rendering was not attempted further because Matplotlib is not installed
in that runtime.
Result: OK (focused radiation damping: 14 passed; examples: 4 passed; smoke
tier: 32 passed, 4 skipped)
```

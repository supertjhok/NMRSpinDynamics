"""Compare tuned CPMG echo decays with diffusion and absolute-phase increments.

This is the tuned-probe counterpart to
``plot_diffusion_absolute_phase_compare.py``.  The tuned probe has visibly
...
from spin_dynamics.workflows import TunedDiffusionCPMGResult  # noqa: E402
from spin_dynamics.workflows import run_tuned_diffusion_cpmg  # noqa: E402
...
def _run_case(
    *,
    ...
    return run_tuned_diffusion_cpmg(
        numpts=int(numpts),
...
def _normalized_integrals(
    result: TunedDiffusionCPMGResult,
    reference: TunedDiffusionCPMGResult,
) -> np.ndarray:
...
def _case_table(args: argparse.Namespace) -> list[tuple[str, str, TunedDiffusionCPMGResult]]:
    ...
    return [
        (
            ...
            "0.30",
            _run_case(**common, diffusion_coefficient=0.0, absolute_phase=None),
        ),
        (
            "Diffusion only",
            "C0",
...
def main() -> None:
    ...
    parser.add_argument("--q-value", type=float, default=50.0, help="Tuned-probe Q.")
    parser.add_argument(
    ...
    print(
        "max absolute-phase residual: "
        f"{max(np.max(phase_delta), np.max(combined_phase_delta)):.6g}"
    )
    ...
    phase_case = combined.absolute_phase
